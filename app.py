...
def build_planning(source_wb, template_wb, values_wb=None):
    ...
    for block_index, block in enumerate(blocks):
        ...
        for i, (src_r, out_r) in enumerate(zip(task_rows_src, task_rows_out)):
            ...
            # Barres 100 % dynamiques. Chaque demi-journée contient une formule
            # invisible qui vaut 1 lorsque la case appartient à la barre, sinon vide.
            # La mise en forme conditionnelle colore uniquement les cellules = 1.
            # Aucune couleur bleue n'est donc écrite en dur : si une durée change,
            # Excel recalcule les cellules et les anciennes cases bleues disparaissent.
            row_range = (
            ...
            first_cal_letter = get_column_letter(calendar_start_col)
            for c in range(calendar_start_col, calendar_end_col + 1):
                cell = ws.cell(out_r, c)
                pos_expr = f"COLUMN()-COLUMN(${first_cal_letter}{out_r})+1"
                cell.value = f'=IF(AND({pos_expr}>{start_expr},{pos_expr}<={end_expr}),1,"")'
                # Cache la valeur 1 : seule la couleur de fond reste visible.
                cell.number_format = ";;;"
...
def process_file(file_bytes: bytes, original_name: str) -> bytes:
    ...
    # Important : ne pas réenregistrer ce .xlsx avec LibreOffice ici.
    # LibreOffice modifie les styles différentiels de mise en forme conditionnelle
    # lors d'une conversion xlsx -> xlsx, ce qui peut supprimer l'affichage bleu
    # dans Excel. Les formules sont marquées pour recalcul automatique à l'ouverture.
