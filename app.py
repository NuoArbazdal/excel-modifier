...
st.markdown(
    ...
    <style>
      ...
      .stDownloadButton > button {
        ...
        font-weight: 700 !important; box-shadow: 0 8px 22px rgba(37,99,235,.18);
...
def build_planning(source_wb, template_wb):
    ...
    for block_index, block in enumerate(blocks):
        ...
        for i, (src_r, out_r) in enumerate(zip(task_rows_src, task_rows_out)):
            ...
            # Les cellules du calendrier restent vides : la couleur est entièrement
            # pilotée par la mise en forme conditionnelle dynamique ci-dessous.
            for c in range(calendar_start_col, calendar_end_col + 1):
                ws.cell(out_r, c).value = None

            # Mise en forme dynamique basée UNIQUEMENT sur la colonne C de cette feuille.
            # La colonne C est elle-même liée à la durée source. Cette approche évite
            # les références inter-feuilles dans les règles de mise en forme conditionnelle,
            # qu'Excel peut ignorer.
            row_range = (
            ...
            inside_formula = (
                f"AND(COLUMN()-COLUMN({first_cal})+1>{start_expr},"
                f"COLUMN()-COLUMN({first_cal})+1<={end_expr})"
            )

            # Une seule règle : bleu quand la demi-journée appartient à la tâche.
            # Quand C change, Excel recalcule la règle et décale automatiquement la suite.
...
def process_file(file_bytes: bytes, original_name: str) -> bytes:
    ...
    # Étape essentielle :
    # openpyxl écrit les formules mais ne calcule pas leurs résultats.
    # On fait donc ouvrir/réenregistrer le classeur par LibreOffice côté serveur.
    # Les valeurs calculées sont ainsi présentes dès le téléchargement, ce qui
    # permet aux règles de mise en forme conditionnelle d'afficher immédiatement
    # les barres bleues. Les formules restent présentes et donc dynamiques.
    libreoffice = shutil.which("libreoffice") or shutil.which("soffice")
    if libreoffice:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            in_path = tmp_path / "planning_a_recalculer.xlsx"
            out_dir = tmp_path / "recalcule"
            out_dir.mkdir()
            in_path.write_bytes(generated_bytes)
            ...
            recalculated_path = out_dir / in_path.name
            if result.returncode == 0 and recalculated_path.exists():
