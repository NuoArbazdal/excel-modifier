...
def build_planning(source_wb, template_wb):
    ...
    for block_index, block in enumerate(blocks):
        ...
        for i, (src_r, out_r) in enumerate(zip(task_rows_src, task_rows_out)):
            ...
            # Barres Gantt :
            # 1) remplissage statique initial pour qu'elles soient visibles immédiatement,
            # 2) mise en forme conditionnelle directe vers les durées SOURCE pour rester dynamiques.
            initial_start = 0.0
            for prev_src_r in task_rows_src[:i]:
                initial_start += as_number(src.cell(prev_src_r, block["duration_col"]).value) * 2
            initial_end = initial_start + as_number(src.cell(src_r, block["duration_col"]).value) * 2

            for c in range(calendar_start_col, calendar_end_col + 1):
                offset = c - calendar_start_col + 1
                cell = ws.cell(out_r, c)
                cell.value = None
                if offset > initial_start and offset <= initial_end:
                    cell.fill = copy(blue_fill)

            # Expressions dynamiques basées directement sur les cellules de durée du fichier source.
            # Ainsi, modifier une durée source redimensionne la barre et décale les suivantes.
            duration_refs_before = [
                excel_ref(src.title, src.cell(r, block["duration_col"]).coordinate)
                for r in task_rows_src[:i]
            ]
            duration_refs_to_current = [
                excel_ref(src.title, src.cell(r, block["duration_col"]).coordinate)
                for r in task_rows_src[: i + 1]
            ]

            if duration_refs_before:
                start_expr = "SUM(" + ",".join(duration_refs_before) + ")*2"
            else:
                start_expr = "0"

            end_expr = "SUM(" + ",".join(duration_refs_to_current) + ")*2"
            row_range = (
            ...
            outside_formula = (
                f"=OR(COLUMN()-COLUMN({first_cal})+1<={start_expr},"
                f"COLUMN()-COLUMN({first_cal})+1>{end_expr})"
            )

            ws.conditional_formatting.add(
                row_range,
                FormulaRule(formula=[outside_formula], fill=white_fill)
            )
            ws.conditional_formatting.add(
                row_range,
                FormulaRule(formula=[inside_formula], fill=blue_fill)
            )
