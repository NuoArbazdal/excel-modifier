...
def build_planning(source_wb, template_wb, values_wb=None):
    ...
    for block_index, block in enumerate(blocks):
        ...
        for i, (src_r, out_r) in enumerate(zip(task_rows_src, task_rows_out)):
            ...
            # Barres dynamiques : les cellules du calendrier restent vides.
            # La couleur bleue est pilotée uniquement par une règle de mise en forme
            # conditionnelle basée sur les durées de la colonne C du planning.
            # Ainsi, si une durée change, la barre se redimensionne et les tâches
            # suivantes se décalent automatiquement.
...
def convert_xls_to_xlsx(file_bytes: bytes, original_name: str) -> bytes:
    ...
    LibreOffice est installé directement dans le Docker sur Render.
    """
    libreoffice = shutil.which("libreoffice") or shutil.which("soffice")
    if not libreoffice:
        raise RuntimeError(
            "Le convertisseur Excel n'est pas disponible sur le serveur. "
            "Vérifie que LibreOffice est bien installé dans le Dockerfile Render."
        )
...
def process_file(file_bytes: bytes, original_name: str) -> bytes:
    ...
    # openpyxl écrit les formules et la mise en forme conditionnelle mais ne calcule
    # pas les résultats des formules. Sur Render, LibreOffice est disponible dans le
    # Docker : on lui fait ouvrir/réenregistrer le classeur une fois afin que les
    # valeurs calculées soient présentes immédiatement au téléchargement.
    # Les règles de mise en forme conditionnelle restent dynamiques dans Excel.
    libreoffice = shutil.which("libreoffice") or shutil.which("soffice")
    if libreoffice:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
